...
-- ============================================
--
-- Migration for adding CIP puzzle support to Darwin Education.
...
-- View for diagnoses with their findings
CREATE OR REPLACE VIEW cip_diagnoses_with_findings AS
SELECT
  d.id,
  d.name_pt,
  d.name_en,
    ...
    (
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'atc_codes', f.atc_codes
        )
        ORDER BY df2.priority
      )
      FROM cip_diagnosis_findings df2
      JOIN cip_findings f ON f.id = df2.finding_id
      WHERE df2.diagnosis_id = d.id
      AND df2.section = df.section
    )
  ) AS findings
FROM cip_diagnoses d
LEFT JOIN cip_diagnosis_findings df ON df.diagnosis_id = d.id
...
COMMENT ON TABLE cip_findings IS 'Clinical findings used as building blocks for CIP puzzles';
COMMENT ON TABLE cip_diagnoses IS 'Medical diagnoses with ICD-10 codes for CIP puzzles';
COMMENT ON TABLE cip_diagnosis_findings IS 'Junction table linking diagnoses to their canonical findings';
COMMENT ON TABLE cip_puzzles IS 'Generated CIP puzzle instances';
COMMENT ON TABLE cip_puzzle_grid IS 'Correct answers for each cell in a CIP puzzle';
COMMENT ON TABLE cip_attempts IS 'User attempts at CIP puzzles with scoring';

COMMENT ON COLUMN cip_findings.section IS 'Which puzzle section this finding belongs to';
COMMENT ON COLUMN cip_diagnoses.difficulty_tier IS '1=very common, 5=rare - affects puzzle difficulty';
COMMENT ON COLUMN cip_puzzles.settings IS 'JSON with diagnosisCount, sections[], distractorCount, allowReuse';
COMMENT ON COLUMN cip_attempts.grid_state IS 'JSON mapping cell keys (row_section) to selected finding IDs';
