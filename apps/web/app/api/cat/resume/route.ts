// ============================================================
// CAT RESUME API ROUTE
// GET /api/cat/resume?sessionId=xxx - Resume an existing CAT session
// ============================================================
...
/**
 * GET /api/cat/resume?sessionId=xxx
 *
 * Resume an existing CAT session and get the next question.
 *
 * Returns: { isComplete, sessionId, config, question?, theta, se, questionNumber }
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (attemptError || !attempt) {
      console.error('Error loading exam attempt:', attemptError)
      return NextResponse.json(
    ...
    // Use default config (could be extended to store config in session)
    const config: CATConfig = DEFAULT_CAT_CONFIG
    ...
    if (session.is_complete) {
      return NextResponse.json({
        ...
        theta: session.theta,
        se: session.se,
        stoppingReason: session.stopping_reason,
      })
    }
    ...
    if (!nextQuestion) {
      // No more items available - session should be marked complete
      return NextResponse.json({
        isComplete: true,
        sessionId,
        theta: session.theta,
    ...
    return NextResponse.json({
      ...
      theta: session.theta,
      se: session.se,
      questionNumber: session.items_administered.length + 1,
    })
  } catch (error) {
    console.error('CAT Resume Error:', error)
    return NextResponse.json(
      {
        error: 'Failed to resume CAT session',
