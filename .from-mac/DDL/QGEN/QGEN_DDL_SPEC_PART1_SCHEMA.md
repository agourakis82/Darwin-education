...
**Data:** 2026-02-01  
**Projeto:** DARWIN/ENAMED Platform  
**Status:** Specification Draft

---

# PARTE 1: SCHEMA DE FEATURES E ANÁLISE DE CORPUS

## 1. Arquitetura Geral do Sistema

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           QGen-DDL ARCHITECTURE                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         DATA LAYER                                   │    │
│  │                                                                      │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐               │    │
│  │  │   CORPUS     │  │  KNOWLEDGE   │  │    USER      │               │    │
│  │  │   STORE      │  │    BASE      │  │   PROFILES   │               │    │
│  │  │  (16k+ Qs)   │  │  (Medical)   │  │  (DDL data)  │               │    │
│  │  └──────────────┘  └──────────────┘  └──────────────┘               │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                    │                                         │
│                                    ▼                                         │
...
│  │                                                                      │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐               │    │
│  │  │   FEATURE    │  │   PATTERN    │  │  MISCONCEPTION│              │    │
│  │  │  EXTRACTOR   │  │   MINER      │  │    MAPPER     │              │    │
│  │  └──────────────┘  └──────────────┘  └──────────────┘               │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                    │                                         │
│                                    ▼                                         │
...
│  │                                                                      │    │
│  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐               │    │
│  │  │   TEMPLATE   │  │     LLM      │  │  DISTRACTOR  │               │    │
│  │  │   ENGINE     │  │  GENERATOR   │  │   CALIBRATOR │               │    │
│  │  └──────────────┘  └──────────────┘  └──────────────┘               │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                    │                                         │
│                                    ▼                                         │
...
CREATE TABLE qgen_vignette_templates (
    ...
    /* Exemplo:
    "[PATIENT_DEMOGRAPHICS], [AGE] anos, [OCCUPATION_IF_RELEVANT],
    procura [SETTING] com queixa de [CHIEF_COMPLAINT] há [TIME_EVOLUTION].
    [ASSOCIATED_SYMPTOMS]. [RELEVANT_HISTORY].
    
    Exame físico: [VITAL_SIGNS]. [PHYSICAL_FINDINGS].
    
    [LAB_RESULTS_IF_NEEDED]
...
BEGIN
    ...
    SELECT 
        q.id,
        1 - (q.embedding <=> p_embedding)::DECIMAL as similarity
    FROM qgen_corpus_questions q
...
BEGIN
    ...
    SELECT 
        ...
        jsonb_object_agg(
            COALESCE(cog.bloom_level::TEXT, 'unknown'),
            count_bloom
        )
    FROM qgen_corpus_questions q
    LEFT JOIN qgen_cognitive_features cog ON q.id = cog.question_id
    LEFT JOIN LATERAL (
        SELECT bloom_level, COUNT(*) as count_bloom
        FROM qgen_cognitive_features
        WHERE question_id = q.id
        GROUP BY bloom_level
    ) bloom_counts ON true
...
## 3. Schema de Features Detalhado (TypeScript)

```typescript
// ============================================================
// QGen-DDL FEATURE SCHEMAS
// src/lib/qgen/types/features.ts
// ============================================================
...
export enum PatientAgeCategory {
  NEONATE = 'NEONATE',           // 0-28 dias
  INFANT = 'INFANT',             // 29 dias - 2 anos
  PRESCHOOL = 'PRESCHOOL',       // 2-6 anos
  SCHOOL_AGE = 'SCHOOL_AGE',     // 6-12 anos
  ADOLESCENT = 'ADOLESCENT',     // 12-18 anos
  YOUNG_ADULT = 'YOUNG_ADULT',   // 18-40 anos
  MIDDLE_ADULT = 'MIDDLE_ADULT', // 40-65 anos
  ELDERLY = 'ELDERLY',           // >65 anos
}
...
export interface StructuralFeatures {
  // Métricas de tamanho do enunciado
  ...
  alternativesWordCounts: Record<string, number>; // {"A": 12, "B": 15}
  ...
  // Formato da pergunta
  questionFormat: string;  // "Qual", "O que", "Como"
  isNegativeStem: boolean; // Contém "EXCETO", "NÃO", "INCORRETA"
...
export interface ClinicalFeatures {
  ...
  patient: {
    ...
    isPregnant: boolean;
    gestationalAge: number | null; // semanas
  };
  ...
  scenario: {
    ...
    setting: string; // "PS", "ambulatório", "UBS"
  };
  ...
  // Sinais vitais
  vitalSigns: {
    bloodPressure: string | null;      // "120x80"
    heartRate: number | null;          // bpm
    respiratoryRate: number | null;    // irpm
  ...
  // Exame físico
  physicalExam: {
    systems: string[];  // ["cardiovascular", "respiratório"]
    findings: Record<string, string>; // {"cardiovascular": "sopro sistólico"}
...
export interface CognitiveFeatures {
  ...
  knowledgeTypes: {
    factual: boolean;      // Fatos, terminologia
    conceptual: boolean;   // Princípios, teorias
    procedural: boolean;   // Como fazer
    metacognitive: boolean; // Conhecer o próprio conhecimento
  };
  ...
  keyConcepts: {
    ...
    required: boolean;  // Obrigatório para resposta correta
    category: string;   // "diagnóstico", "tratamento", "fisiopatologia"
  }[];
...
export interface LinguisticFeatures {
  // Marcadores de hedging
  hedging: {
    count: number;
    markers: string[];  // ["pode", "geralmente", "frequentemente"]
    density: number;    // marcadores / palavras totais
  };
  
  // Termos absolutos
  absoluteTerms: {
    count: number;
    markers: string[];  // ["sempre", "nunca", "único", "todos"]
    inCorrectAnswer: boolean; // Flag de problema
  };
  ...
  // Problemas/pistas linguísticas
  linguisticCues: {
    grammaticalCues: boolean;  // Concordância que revela
    lengthCue: boolean;        // Correta muito maior/menor
    absoluteInCorrect: boolean; // Termo absoluto na correta
    issues: string[];
  };
  ...
  readability: {
    ...
    gunningFogIndex: number;    // Anos de escolaridade
...
/**
 * Objeto completo de features extraídas
 */
export interface ExtractedFeatures {
  ...
  clinical: ClinicalFeatures | null;  // Null se não for caso clínico
  cognitive: CognitiveFeatures;
  linguistic: LinguisticFeatures;
  distractors: DistractorFeatures[];
  
  // IRT estimado/calculado
  irt: {
    difficulty: number;      // -3 a +3
    discrimination: number;  // 0 a 2+
    guessing: number;        // ~0.25 para 4 alt
  ...
  extractionMetadata: {
    ...
    warnings: string[];      // Problemas encontrados
...
/**
 * Padrões regex para extração de features linguísticas
 */
...
/**
 * Padrões para extração de dados clínicos
 */
export const CLINICAL_PATTERNS = {
  ...
  sex: {
    ...
    female: /\b(mulher|feminino|sexo feminino|paciente do sexo feminino|menina|garota|gestante|puérpera)\b/i,
  },
  ...
  vitalSigns: {
    ...
    hr: /FC\s*[:=]?\s*(\d+)/i,
    rr: /FR\s*[:=]?\s*(\d+)/i,
    temp: /T(?:ax|emp)?\s*[:=]?\s*(\d+[,.]?\d*)/i,
    spo2: /(?:SpO2|SatO2|Saturação)\s*[:=]?\s*(\d+)/i,
  },
  
  // Cenários
  scenarios: {
    emergency: /\b(pronto.?socorro|PS|emergência|urgência|UPA)\b/i,
    outpatient: /\b(ambulat[óo]rio|consulta|consult[óo]rio)\b/i,
    inpatient: /\b(enfermaria|internado|internação|leito)\b/i,
    icu: /\b(UTI|terapia intensiva|CTI)\b/i,
    primaryCare: /\b(UBS|posto de saúde|atenção primária|ESF|UBSF)\b/i,
  },
};

/**
 * Mapeamento de termos médicos para conceitos
 */
...
Este é o primeiro arquivo (Parte 1) contendo o schema de banco de dados e as interfaces TypeScript para extração de features. Vou continuar com as partes seguintes.
