-- ============================================
-- CIP Achievements System Schema
-- ============================================
--
-- Run this in Supabase SQL Editor to add achievements to CIP
--
-- Tables:
-- - cip_achievements: Achievement definitions
-- - user_cip_achievements: User's unlocked achievements
-- ============================================

-- ============================================
-- CIP Achievements Table (Master List)
-- ============================================
...
CREATE TABLE IF NOT EXISTS cip_achievements (
  ...
  -- Rarity/Tier (affects visual styling)
...
-- ============================================
-- User CIP Achievements Table (Progress)
-- ============================================
...
-- Public read for achievement definitions
...
-- Users cannot manually insert achievements
CREATE POLICY user_cip_achievements_insert ON user_cip_achievements
  FOR INSERT WITH CHECK (auth.uid() = user_id);

-- Users can mark achievements as notified
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE scaled_score >= 800) as high_scores,
    COUNT(*) FILTER (WHERE passed = true) as passed_puzzles
  INTO v_user_stats
  FROM cip_attempts
  ...
  LOOP
    ...
    CASE v_achievement.achievement_type
      ...
      WHEN 'perfect_score' THEN
        IF (v_attempt.correct_count::float / NULLIF(v_attempt.total_cells, 0)) >= 1.0 THEN
          INSERT INTO user_cip_achievements (user_id, achievement_id, related_attempt_id, metadata)
      ...
      WHEN 'speed' THEN
        IF v_attempt.total_time_seconds <= (v_achievement.criteria->>'time_seconds')::int THEN
          INSERT INTO user_cip_achievements (user_id, achievement_id, related_attempt_id, metadata)
...
-- ============================================
-- Trigger to Check Achievements on Attempt Completion
-- ============================================

CREATE OR REPLACE FUNCTION trigger_check_cip_achievements()
RETURNS TRIGGER AS $$
BEGIN
  -- Only check achievements when attempt is completed
  IF NEW.completed_at IS NOT NULL AND OLD.completed_at IS NULL THEN
    -- Call the achievement checker
...
-- To test:
-- 1. Complete a puzzle
-- 2. Query: SELECT * FROM check_cip_achievements('YOUR_USER_ID', 'YOUR_ATTEMPT_ID');
-- 3. View your achievements: SELECT * FROM user_cip_achievement_progress WHERE user_id = 'YOUR_USER_ID';
